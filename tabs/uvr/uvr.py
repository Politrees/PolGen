...
# Список моделей для различных архитектур
ROFORMER_MODELS = {
...
def uvr_tab():
        ...
        with gr.Group():
            ...
            with gr.Row():
                output_dir = gr.Textbox(
                    value="output",
                    label="Путь для сохранения результатов",
                    info="Директория, где будут сохранены выходные файлы.",
                    placeholder="output",
                )
            ...
            with gr.Row():
                batch_size = gr.Slider(
                    ...
                    value=1,
                    label="Размер пакета",
                    info="Больший размер пакета потребляет больше памяти, но может обрабатывать быстрее.",
                )
    ...
    with gr.Tab("Roformer"):
        with gr.Group():
            ...
            with gr.Row():
                roformer_seg_size = gr.Slider(
                    ...
                    info="Больший размер потребляет больше ресурсов, но может дать лучшие результаты.",
                )
                ...
                roformer_overlap = gr.Slider(
                    ...
                    value=8,
                    label="Перекрытие",
                    info="Количество перекрытия между окнами прогнозирования. Меньшее значение лучше, но медленнее.",
                )
                roformer_pitch_shift = gr.Slider(
                    ...
                    info="Сдвиг высоты тона аудио на несколько полутонов во время обработки. Может улучшить выход для глубоких/высоких вокалов.",
                )
        with gr.Row():
        ...
        with gr.Row():
            roformer_stem1 = gr.Audio(
                label="Дорожка 1", type="filepath", interactive=False
            )
            roformer_stem2 = gr.Audio(
                label="Дорожка 2", type="filepath", interactive=False
            )
        ...
        with gr.Group():
            ...
            with gr.Row():
                ...
                mdx23c_overlap = gr.Slider(
                    ...
                    value=8,
                    label="Перекрытие",
                    info="Количество перекрытия между окнами прогнозирования. Большее значение лучше, но медленнее.",
                )
    ...
    with gr.Tab("MDX-NET"):
        with gr.Group():
            ...
            with gr.Row():
                mdx_hop_length = gr.Slider(
                    ...
                    value=1024,
                    label="Длина шага",
                    info="Обычно называется шагом в нейронных сетях; изменяйте только если знаете, что делаете.",
                )
                ...
                mdx_denoise = gr.Checkbox(
                    ...
                    info="Включить шумоподавление после разделения.",
                )
        with gr.Row():
        ...
        with gr.Row():
            mdx_stem1 = gr.Audio(label="Дорожка 1", type="filepath", interactive=False)
            mdx_stem2 = gr.Audio(label="Дорожка 2", type="filepath", interactive=False)
    ...
    with gr.Tab("VR ARCH"):
        with gr.Group():
            ...
            with gr.Row():
                vr_window_size = gr.Slider(
                    ...
                    value=512,
                    label="Размер окна",
                    info="Баланс между качеством и скоростью. 1024 = быстро, но ниже качество, 320 = медленнее, но лучше качество.",
                )
                vr_aggression = gr.Slider(
                    ...
                    value=5,
                    label="Агрессивность",
                    info="Интенсивность извлечения основной дорожки.",
                )
                vr_tta = gr.Checkbox(
                    value=False,
                    label="TTA",
                ...
                vr_post_process = gr.Checkbox(
                    value=False,
                    label="Постобработка",
                    info="Определить оставшиеся артефакты в вокальном выходе; может улучшить разделение для некоторых песен.",
                )
                ...
                vr_high_end_process = gr.Checkbox(
                    ...
                    info="Отразить отсутствующий диапазон частот выхода.",
                )
        with gr.Row():
        ...
        with gr.Row():
            vr_stem1 = gr.Audio(label="Дорожка 1", type="filepath", interactive=False)
            vr_stem2 = gr.Audio(label="Дорожка 2", type="filepath", interactive=False)
    ...
    with gr.Tab("Demucs"):
        with gr.Group():
            ...
            with gr.Row():
                demucs_seg_size = gr.Slider(
                    ...
                    value=40,
                    label="Размер сегмента",
                    info="Размер сегментов, на которые делится аудио. Большее значение = медленнее, но лучше качество.",
                )
                demucs_shifts = gr.Slider(
                    ...
                    value=2,
                    label="Сдвиги",
                    info="Количество прогнозов со случайными сдвигами, большее значение = медленнее, но лучше качество.",
                )
                demucs_overlap = gr.Slider(
                    ...
                    value=0.25,
                    label="Перекрытие",
                    info="Перекрытие между окнами прогнозирования. Большее значение = медленнее, но лучше качество.",
                )
                demucs_segments_enabled = gr.Checkbox(
                    value=True,
                    label="Обработка по сегментам",
                    info="Включить обработку по сегментам.",
                )
        with gr.Row():
        ...
        with gr.Row():
            demucs_stem3 = gr.Audio(
                label="Дорожка 3", type="filepath", interactive=False
            )
            demucs_stem4 = gr.Audio(
                label="Дорожка 4", type="filepath", interactive=False
            )
        with gr.Row(visible=False) as stem6:
            demucs_stem5 = gr.Audio(
                label="Дорожка 5", type="filepath", interactive=False
            )
            demucs_stem6 = gr.Audio(
                label="Дорожка 6", type="filepath", interactive=False
            )
