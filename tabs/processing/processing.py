                        ...
                        with gr.Column(variant="panel"):
                            with gr.Row():
                                reverb_rm_size = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за размер виртуального "
                                    "помещения, в котором будет звучать реверберация. "
                                    "Большее значение означает больший размер комнаты и "
                                    "более длительное звучание реверберации.",
                                )
                                reverb_width = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за ширину звучания "
                                    "реверберации. Чем выше значение, тем шире будет "
                                    "звучание реверберации.",
                                )
                            with gr.Row():
                                reverb_wet = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за уровень "
                                    "реверберации. Чем выше значение, тем сильнее "
                                    "будет слышен эффект реверберации и тем дольше "
                                    "будет звучать «хвост».",
                                )
                                reverb_dry = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за уровень исходного "
                                    "звука без реверберации. Чем меньше значение, тем "
                                    "тише звук ai вокала. Если значение будет на 0, то "
                                    "исходный звук полностью исчезнет.",
                                )
                            with gr.Row():
                                reverb_damping = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за поглощение высоких "
                                    "частот в реверберации. Чем выше его значение, тем "
                                    "сильнее будет поглощение частот и тем менее будет "
                                    "«яркий» звук реверберации.",
                                )
                        ...
                        with gr.Column(variant="panel"):
                            with gr.Row():
                                chorus_rate_hz = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за скорость колебаний "
                                    "эффекта хоруса в герцах. Чем выше значение, тем "
                                    "быстрее будут колебаться звуки.",
                                )
                                chorus_depth = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за глубину эффекта "
                                    "хоруса. Чем выше значение, тем сильнее будет "
                                    "эффект хоруса.",
                                )
                            with gr.Row():
                                chorus_centre_delay_ms = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за задержку "
                                    "центрального сигнала эффекта хоруса в "
                                    "миллисекундах. Чем выше значение, тем дольше "
                                    "будет задержка.",
                                )
                                chorus_feedback = gr.Slider(
                                    ...
                                    info="Этот параметр отвечает за уровень обратной "
                                    "связи эффекта хоруса. Чем выше значение, тем "
                                    "сильнее будет эффект обратной связи.",
                                )
                            with gr.Row():
                                chorus_mix = gr.Slider(
                                    ...
                                    value=0,
                                    label="Смешение",
                                    info="Этот параметр отвечает за уровень смешивания "
                                    "оригинального сигнала и эффекта хоруса. Чем выше "
                                    "значение, тем сильнее будет эффект хоруса.",
                                )
                ...
                with gr.Accordion("Компрессор", open=False):
                    with gr.Row(variant="panel"):
                        compressor_ratio = gr.Slider(
                            ...
                            value=4,
                            label="Соотношение",
                            info="Этот параметр контролирует количество применяемого "
                            "сжатия аудио. Большее значение означает большее сжатие, "
                            "которое уменьшает динамический диапазон аудио, делая "
                            "громкие части более тихими и тихие части более громкими.",
                        )
                        compressor_threshold = gr.Slider(
                            ...
                            value=-12,
                            label="Порог",
                            info="Этот параметр устанавливает порог, при превышении "
                            "которого начинает действовать компрессор. Компрессор "
                            "сжимает громкие звуки, чтобы сделать звук более ровным. "
                            "Чем ниже порог, тем большее количество звуков будет "
                            "подвергнуто компрессии.",
                        )
                ...
                with gr.Accordion("Фильтры", open=False):
                    with gr.Row(variant="panel"):
                        low_shelf_gain = gr.Slider(
                            ...
                            info="Этот параметр контролирует усиление (громкость) "
                            "низких частот. Положительное значение усиливает низкие "
                            "частоты, делая звук более басским. Отрицательное значение "
                            "ослабляет низкие частоты, делая звук более тонким.",
                        )
                        high_shelf_gain = gr.Slider(
                            ...
                            info="Этот параметр контролирует усиление высоких частот. "
                            "Положительное значение усиливает высокие частоты, делая "
                            "звук более ярким. Отрицательное значение ослабляет высокие "
                            "частоты, делая звук более тусклым.",
                        )
                        ...
                        with gr.Column(variant="panel"):
                            with gr.Row():
                                noise_gate_threshold = gr.Slider(
                                    ...
                                    value=-40,
                                    label="Порог",
                                    info="Этот параметр устанавливает пороговое "
                                    "значение в децибелах, ниже которого сигнал "
                                    "считается шумом. Когда сигнал опускается ниже "
                                    "этого порога, шумовой шлюз активируется и "
                                    "уменьшает громкость сигнала.",
                                )
                                noise_gate_ratio = gr.Slider(
                                    ...
                                    value=8,
                                    label="Соотношение",
                                    info="Этот параметр устанавливает уровень "
                                    "подавления шума. Большее значение означает более "
                            ...
                            with gr.Row():
                                noise_gate_attack = gr.Slider(
                                    ...
                                    info="Этот параметр контролирует скорость, с "
                                    "которой шумовой шлюз открывается, когда звук "
                                    "становится достаточно громким. Большее значение "
                                    "означает, что шлюз открывается медленнее.",
                                )
                                noise_gate_release = gr.Slider(
                                    ...
                                    info="Этот параметр контролирует скорость, с "
                                    "которой шумовой шлюз закрывается, когда звук "
                                    "становится достаточно тихим. Большее значение "
                                    "означает, что шлюз закрывается медленнее.",
                                )
