...
def save_audio(audio_data, sample_rate, output_path, output_format="wav", stereo=False):
    ...
    cmd = [
        ...
        '-f', input_format,  # Формат входных данных
        '-ar', str(sample_rate),  # Частота дискретизации
        '-ac', '1',  # Входные каналы (всегда моно из RVC)
        '-i', 'pipe:0',  # Читать из stdin
        '-ac', str(channels),  # Выходные каналы
    ]

    # Настройки качества для каждого формата
    format_settings = {
        'wav': [
            '-c:a', 'pcm_f32le',  # 32-bit float PCM для максимального качества
            '-sample_fmt', 'flt',
        ],
        'flac': [
            '-c:a', 'flac',
            '-compression_level', '12',  # Максимальное сжатие (без потерь)
            '-sample_fmt', 's32',  # 32-bit для максимального качества
        ],
        'mp3': [
            '-c:a', 'libmp3lame',
            '-b:a', '320k',  # Максимальный битрейт
            '-q:a', '0',  # Наилучшее качество
        ],
        'ogg': [
            '-c:a', 'libvorbis',
            '-q:a', '10',  # Максимальное качество (500kbps+)
        ],
        'm4a': [
            '-c:a', 'aac',
            '-b:a', '320k',  # Максимальный битрейт
            '-q:a', '2',  # Максимальное качество
            '-aac_coder', 'twoloop',  # Лучший кодировщик
            '-profile:a', 'aac_low',
        ],
    }
